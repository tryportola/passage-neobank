...
var WebhookHandler = class {
  ...
  verifySignature(payload, signature) {
    ...
    const expectedSig = crypto__default.default.createHmac("sha256", this.secret).update(signedPayload).digest("hex");
    const sigBuffer = Buffer.from(sig, "hex");
    const expectedSigBuffer = Buffer.from(expectedSig, "hex");
    if (sigBuffer.length !== expectedSigBuffer.length || !crypto__default.default.timingSafeEqual(sigBuffer, expectedSigBuffer)) {
  ...
  generateTestHeaders(payload) {
    ...
    const signedPayload = `${timestamp}.${payload}`;
    const signature = crypto__default.default.createHmac("sha256", this.secret).update(signedPayload).digest("hex");
    return {
